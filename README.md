...
面向 Codex 与兼容 Agent 工作流的三个独立 Godot 技能。每个技能都可以单独安装、单独升级，不依赖本仓库中的其他技能。

| 技能 | 用途 | 运行要求 |
| --- | --- | --- |
| `godot-dev` | 使用 Godot 4.7 官方契约、文本资产、反射和 CLI 开发、审查与排错 | Godot 4.7.1 |
...
```powershell
python "$env:CODEX_HOME\skills\.system\skill-installer\scripts\install-skill-from-github.py" --repo ZhaoMCX/godot-skills --path godot-dev
...
CI 还会使用 Godot 4.7.1 运行 `godot-dev` 的完整清单、自测和夹具验证。`godot-framework` 的契约测试是自包含的 Godot CLI 测试，不要求 GUT；消费项目自己的测试框架不受限制。
